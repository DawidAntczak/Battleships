...
namespace Battleships.Core.Ships
{
    public record Ship
    {
        public IEnumerable<Position> OccupiedPositions { get; }

        public Ship(Position start, Position end)
        {
            var topLeft = new Position(Math.Min(start.Row, end.Row), Math.Min(start.Column, end.Column));
            var bottomRight = new Position(Math.Max(start.Row, end.Row), Math.Max(start.Column, end.Column));
